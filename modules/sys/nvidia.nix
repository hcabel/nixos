...
in
{
  ...
  hardware.nvidia = {
    # Proprietary kernel modules, not the open ones. Ada is Turing+ so the open
    # modules would otherwise be the natural pick, but DaVinci Resolve's
    # CUDA/OpenGL interop fails against them here: cudaGraphicsGLRegisterImage
    # returns cudaErrorUnknown on every frame, which Resolve surfaces as a bogus
    # "out of GPU memory". Blackmagic only claim support for the proprietary
    # modules.
    open = false;
